...
dsc { 'configure_jenkins_xml':
  ...
  properties    => {
    ...
    setscript  => '$p = "C:\\Program Files\\Jenkins\\jenkins.xml"; $newArgs = "--httpPort=8000 --webroot=`"%LocalAppData%\\Jenkins\\war`" -Djenkins.install.runSetupWizard=false"; $jvmOpts = "-Xrs -Xmx256m -Dhudson.lifecycle=hudson.lifecycle.WindowsServiceLifecycle -jar `"C:\\Program Files\\Jenkins\\jenkins.war`""; if (Test-Path $p) { $xml = Get-Content $p -Raw; $xml = $xml -replace "version=`"1.1`"","version=`"1.0`""; $xml = $xml -replace "(?s)<jvmOptions>.*?</jvmOptions>`r`n  ",""; $xml = $xml -replace "(?s)<arguments>.*?</arguments>","<jvmOptions>$jvmOpts</jvmOptions>`r`n  <arguments>$newArgs</arguments>"; Set-Content -Path $p -Value $xml -Encoding UTF8 } else { $template = "<?xml version=`"1.0`" encoding=`"UTF-8`"?>`r`n<service>`r`n  <id>Jenkins</id>`r`n  <name>Jenkins</name>`r`n  <description>Jenkins Automation Server</description>`r`n  <executable>%BASE%\\jenkins.exe</executable>`r`n  <jvmOptions>$jvmOpts</jvmOptions>`r`n  <arguments>$newArgs</arguments>`r`n  <logmode>rotate</logmode>`r`n</service>"; Set-Content -Path $p -Value $template -Encoding UTF8 }',
  },
